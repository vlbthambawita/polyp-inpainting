...
from data.data import InpaintingDataset_WithMask_v2, ToTensor
from model.net import InpaintingModel_GMCNN_Given_Mask
from options.train_options_kvasir import TrainOptionsGivenMask
from util.utils import getLatest

#torch.cuda.set_device(1)

train_option = TrainOptionsGivenMask()
...
print('loading data..')
dataset = InpaintingDataset_WithMask_v2(config.dataset_path, config.data_root, transform=transforms.Compose([
    ToTensor()
]))
...
num_of_layers_to_freeze = 10

for i,child in enumerate(ourModel.netGM.EB1.children()):#parameters()): #named_parameters():
    print(i)
    print(child)
    if i < num_of_layers_to_freeze:
        for param in child.parameters():
            param.requires_grad = False
            print("freezed child = {}".format(i))

for i,child in enumerate(ourModel.netGM.EB2.children()):#parameters()): #named_parameters():
    print(i)
    print(child)
    if i < num_of_layers_to_freeze:
        for param in child.parameters():
...
for i,child in enumerate(ourModel.netGM.EB3.children()):#parameters()): #named_parameters():
    print(i)
    print(child)
    if i < num_of_layers_to_freeze:
        for param in child.parameters():
...
print("cheking autograds..")
for i,child in enumerate(ourModel.netGM.EB1.parameters()):#parameters()): #named_parameters():
    print(i)
    print(child)
